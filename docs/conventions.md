...
## When to read
- 코딩 컨벤션 확인 (Java, Python, TS)
- 에러 핸들링 규칙 확인
- Git commit / PR 컨벤션 확인


> **Harness Role:** 서비스별 코딩 방식과 상태·의존성 선택 기준을 제공합니다. Agent는 새 패턴이나 라이브러리를 도입하기 전에 읽습니다. 이 문서가 없으면 같은 문제를 서로 다른 방식으로 구현하거나 불필요한 도구를 추가할 수 있습니다. 실제 준수 여부는 build 설정과 `check-format.sh`, `check-lint.sh`, 서비스별 검사에서 확인합니다.
