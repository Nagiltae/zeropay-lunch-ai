...
현재 자연어 처리는 `AiIntentAnalyzer` 경계 뒤의 제한된 임시 키워드 규칙입니다. 사용자 인증, 취향과 식사 기록은 구현되었지만 실제 음식점 데이터, FastAPI HTTP 클라이언트와 서버 구현, LLM, Qdrant 검색과 LangGraph는 아직 구현되지 않았습니다.
