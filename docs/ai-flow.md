...
현재 FastAPI에는 상태 확인 엔드포인트만 있습니다. FastAPI 연결 전 흐름 검증을 위해 Spring Boot가 제한된 키워드를 임시 구조로 변환하고, MySQL에서 현재 영업 중인 샘플 음식점을 조회해 결정론적으로 순위를 정합니다. 이 임시 분석기는 향후 FastAPI의 검증된 구조화 의도 분석으로 교체하지만 영업시간 필터와 최종 순위는 계속 Spring Boot가 담당합니다.
